              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-900">
                    {nextShift.position ? (typeof nextShift.position === 'object' && nextShift.position !== null && 'name' in nextShift.position ? (nextShift.position as { name: string }).name : String(nextShift.position)) : 'Schicht'}
                  </p>
                      ...
                      {activeEntry.shift && (
                        <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                          ...
                          {activeEntry.shift.position && (
                            <p className="text-xs text-blue-600 mt-1">Position: {typeof activeEntry.shift.position === 'object' && activeEntry.shift.position !== null && 'name' in activeEntry.shift.position ? (activeEntry.shift.position as { name: string }).name : String(activeEntry.shift.position || '')}</p>
                          )}
                        </div>
                      )}
                    </>
                    ...
                    <div className="flex-1">
                      ...
                      {entry.shift && (
                        <p className="text-xs text-gray-500 mt-1 ml-5">
                          Schicht: {entry.shift.position ? (typeof entry.shift.position === 'object' && entry.shift.position !== null && 'name' in entry.shift.position ? (entry.shift.position as { name: string }).name : String(entry.shift.position)) : 'Schicht'} ({formatTime(entry.shift.start_time)} - {formatTime(entry.shift.end_time)})
                        </p>
                      )}
                    </div>
                    <div className="flex items-center space-x-2">
